...
async def search(chat_id, words, user):
    client = Client('me_client_bot', 9006209, '57f2dfbe60fb31b1f7ef85ec937f36b5')

    await client.start()
    ans = []
    if (user):
        async for x in client.search_messages(chat_id, query=words, from_user=user):
            ans.append({"id": x.id, "from_user": x.from_user, "text": x.text})
    else: 
        async for x in client.search_messages(chat_id, query=words):
            ans.append({"id": x.id, "from_user": x.from_user, "text": x.text})
    await client.stop()
